				...
				else
				{
					...
					else
					{
						...
						while (offset < pattern.Length && char.IsDigit(pattern[offset]))
						{
							...
							formattingInfo.Min = (formattingInfo.Min * 10) + int.Parse(pattern[offset].ToString(CultureInfo.InvariantCulture), NumberFormatInfo.InvariantInfo);
							offset++;
						}
						...
						while (offset < pattern.Length && char.IsDigit(pattern[offset]))
						{
							...
							formattingInfo.Max = (formattingInfo.Max * 10) + int.Parse(pattern[offset].ToString(CultureInfo.InvariantCulture), NumberFormatInfo.InvariantInfo);
							offset++;
						}
						...
						for(int m=0; m<matches.Length; m++)
						{
							...
							{
								if (String.Compare(pattern, offset, matches[m], 0, matches[m].Length, false, CultureInfo.InvariantCulture) == 0)
								{
									// Found match
