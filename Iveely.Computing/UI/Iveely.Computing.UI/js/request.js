...
function buildCommand(command, topology) {
...
function sendCommand(command, topology) {
    if (uiSocket != null) {
        try {
            var data = buildCommand(command, topology);
            uiSocket.send(data);
            return true;
        } catch (e) {
            alert(e);
...
function showClusterSummary() {
    sendCommand('cluster summary', 'empty');
};

function showTopologySummary() {
    sendCommand('topology summary', 'empty');
};

function showSlaveSummary() {
    sendCommand('slave summary', 'empty');
};

function showSystemSummary() {
    sendCommand('system summary', 'empty');
};

function queryTolopology() {
    sendCommand('query topology', getQueryString('name'));
};

function taskSummary() {
    sendCommand('task summary', getQueryString('name'));
};

function statisticSummary() {
    sendCommand('statistic summary', getQueryString('name'));
};

function killTopology() {
    if (confirm("Are you sure to kill this topology?")) {
        sendCommand('kill', getQueryString('name'));
    }
}
