...
public class Dispatcher {
     ...
     * @return
     */
    public InternetPacket callSlaves(InternetPacket packet) {
        ...
        if (executType == Message.ExecutType.UPLOAD) {
            return processTask(packet, Message.ExecutType.RESPUPLOADAPP);
        }
        ...
        if (executType == Message.ExecutType.LIST) {
            return processTask(packet, Message.ExecutType.RESPLISTTASK);
        }

        // 4. Kill task.
        if (executType == Message.ExecutType.KILLTASK) {
            return processTask(packet, Message.ExecutType.RESPKILLTASK);
        }
     ...
     * @return
     */
    private InternetPacket processRunApp(InternetPacket packet) {
     ...
     * @return
     */
    private InternetPacket processTask(InternetPacket packet, Message.ExecutType respExecutType) {
