...
import com.iveely.framework.Media.Thumbnail;
...
public class Html2Article {
     ...
     * @param page
     */
    public static void getArticle(HtmlPage page) {
        String html = page.getCode();
        if (html == null) {
            return;
        }
        if (stringNumbers(0, html, "\n") < 10) {
            html = html.replace(">", ">\n").toLowerCase();
        }
        String body = "";
        ...
        if (!(page.isIsHost() && getMetaDesc(page))) {
            String content = null;
            ...
            if (content.length() < 20) {
                content = html.toLowerCase().replaceAll("<script(?:[^<]++|<(?!/script>))*+</script>", "").replaceAll("<style(?:[^<]++|<(?!/style>))*+</style>", "").replaceAll("<.*?>", "").trim();
            } else {
                content = content.toLowerCase().replaceAll("<script(?:[^<]++|<(?!/script>))*+</script>", "").replaceAll("<style(?:[^<]++|<(?!/style>))*+</style>", "").replaceAll("<.*?>", "").trim();
            }
            contentWithTags = tempRef_contentWithTags.argvalue;
            page.SetContent(content);
        }
        page.SetTitle(getTitle(html, null));
        page.SetPublishDate(getPublishDate(body));
    }
     ...
     * @return
     */
    public static boolean getMetaDesc(HtmlPage page) {
        String code = page.getCode().toLowerCase();
        ...
        while (matcher.find()) {
            ...
            if (temp.contains("description")) {
                page.SetContent(temp.replace("content=", "").replace("<meta", "").replace("\"", "").replace("/>", "").trim().replace("name=description", "").replace(">", ""));
                return true;
            }
        }
        return false;
     ...
     * @return
     */
    private static String getTitle(String html, String backupTitle) {
     ...
     * @return
     */
    public static List<Image> getAllImages(int pageId, String fromUrl, String htmlCode) {
        ...
        final Matcher mt = pt.matcher(htmlCode);
        while (mt.find()) {
            final Matcher urlMatcher = Pattern.compile("src=.*?>", Pattern.DOTALL).matcher(mt.group());
            String childUrl = "";
            while (urlMatcher.find()) {
                childUrl = urlMatcher.group();
                if (childUrl.contains("alt=")) {
                    String[] urlComs = childUrl.split(" ");
                    String urlCom = urlComs[0].replaceAll("src=|>", "");
                    if (urlCom.length() > 5) {
                        childUrl = joinUrl(fromUrl, urlCom);
                    }
                    String alt = "";
                    for (int i = 1; i < urlComs.length; i++) {
                        if (urlComs[i].startsWith("alt")) {
                            alt = urlComs[i].replace("alt=", "").replace("\"", "");
                        }
                    }
                    if (alt.length() < 3) {
                        continue;
                    ...
                    if (childUrl.startsWith("http")) {
                        logger.info("image url:" + childUrl + "[" + alt + "]");
                        String base64 = Thumbnail.getImageBase64FromNet(childUrl, 200, 200, true);
                        if (base64 != null && base64.length() > 100) {
                            Image image = new Image();
                            if (image.setImage(base64)) {
                                image.setAlt(alt.replace("/>", "").replace(">", ""));
                                image.setId(pageId);
                                image.setUrl(childUrl);
                                image.setPageUrl(fromUrl);
